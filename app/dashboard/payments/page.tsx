...
function PaymentsPage(){
 ...
 const [search,setSearch]=useState("");const [debouncedSearch,setDebouncedSearch]=useState("");const [directionFilter,setDirectionFilter]=useState("");const [statusFilter,setStatusFilter]=useState("");const [error,setError]=useState("");const [success,setSuccess]=useState("");const [mode,setMode]=useState<"create"|"view"|"edit"|"receive"|null>(null);const [selected,setSelected]=useState<Payment|null>(null);const [form,setForm]=useState(empty);const [saving,setSaving]=useState(false);
 ...
 const listParams=useMemo(()=>({search:debouncedSearch.trim()||undefined,direction:directionFilter||undefined,tx_status:statusFilter||undefined}),[debouncedSearch,directionFilter,statusFilter]);
 const paymentsQuery=useQuery({
   queryKey:queryKeys.payments.list(listParams),
   queryFn:()=>{const p=new URLSearchParams();if(listParams.search)p.set("search",listParams.search);if(listParams.direction)p.set("direction",listParams.direction);if(listParams.tx_status)p.set("tx_status",listParams.tx_status);p.set("limit","300");return apiFetch<ListResponse>(`/payments?${p}`);},
   staleTime:90*1000,gcTime:10*60*1000,placeholderData:keepPreviousData,
 });
 const rows=paymentsQuery.data?.items??[];
 const loading=paymentsQuery.isLoading;
 const load=()=>paymentsQuery.refetch();
 const customersLookupQuery=useQuery({queryKey:queryKeys.payments.lookup("customers"),queryFn:()=>apiFetch<Lookup[]>("/payments/lookups/customers"),staleTime:5*60*1000});
 const customers=customersLookupQuery.data??[];
 const suppliersLookupQuery=useQuery({queryKey:queryKeys.payments.lookup("suppliers"),queryFn:()=>apiFetch<Lookup[]>("/payments/lookups/suppliers"),staleTime:5*60*1000});
 const suppliers=suppliersLookupQuery.data??[];
 const salesOrdersLookupQuery=useQuery({queryKey:queryKeys.payments.lookup("sales-orders"),queryFn:()=>apiFetch<Lookup[]>("/payments/lookups/sales-orders"),staleTime:2*60*1000});
 const salesOrders=salesOrdersLookupQuery.data??[];
 const purchaseOrdersLookupQuery=useQuery({queryKey:queryKeys.payments.lookup("purchase-orders"),queryFn:()=>apiFetch<Lookup[]>("/payments/lookups/purchase-orders"),staleTime:2*60*1000});
 const purchaseOrders=purchaseOrdersLookupQuery.data??[];
 const shipmentsLookupQuery=useQuery({queryKey:queryKeys.payments.lookup("shipments"),queryFn:()=>apiFetch<Lookup[]>("/payments/lookups/shipments"),staleTime:2*60*1000});
 const shipments=shipmentsLookupQuery.data??[];
 const containersLookupQuery=useQuery({queryKey:queryKeys.payments.lookup("containers"),queryFn:()=>apiFetch<Lookup[]>("/payments/lookups/containers"),staleTime:2*60*1000});
 const containers=containersLookupQuery.data??[];
